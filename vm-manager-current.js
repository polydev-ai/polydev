...
class VMManager {
   ...
   */
  /**
   * Inject runtime configuration (Proxy + Session ID)
   * Simplified for Golden Rootfs (no file copying needed)
   */
  async injectOAuthAgent(vmId, rootfsPath, userId = null, sessionId = null) {
    const mountPoint = `/tmp/vm-inject-${vmId}`;

    try {
      logger.info('[INJECT-AGENT] Starting runtime configuration injection', { vmId, rootfsPath, mountPoint });
      ...
      // Inject Decodo proxy configuration + SESSION_ID
      if (userId) {
        try {
...
`;
          ...
          logger.info('[INJECT-AGENT] /etc/environment injected successfully');

          // Also inject into profile.d for shell sessions
          const profileContent = `export HTTP_PROXY="${proxyEnv.HTTP_PROXY}"
...
export no_proxy="${proxyEnv.NO_PROXY}"
`;
          fsSync.writeFileSync(path.join(mountPoint, 'etc/profile.d/decodo-proxy.sh'), profileContent);
          execSync(`chmod +x "${path.join(mountPoint, 'etc/profile.d/decodo-proxy.sh')}"`, { stdio: 'pipe' });

        } catch (proxyError) {
          logger.error('[INJECT-AGENT] Failed to inject proxy configuration', { error: proxyError.message });
        }
      }
      ...
      logger.error('[INJECT-AGENT] Injection failed', { vmId, error: error.message });
      throw error;
    } finally {
      // Always unmount
      try {
        execSync(`umount -f "${mountPoint}"`, { stdio: 'pipe' });
        execSync(`rmdir "${mountPoint}"`, { stdio: 'pipe' });
      } catch (e) {
        // Ignore cleanup errors
      }
