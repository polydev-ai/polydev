    ...
    <div className={`${showSidebar ? 'w-80' : 'w-0'} transition-all duration-300 overflow-hidden bg-slate-50 border-r border-slate-200`}>
      <div className="p-4">
        ...
        <button
          ...
          className="w-full mb-4 px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 transform"
        >
          ...
          {sessions.map((session) => (
            <div key={session.id} className={`group flex items-center justify-between p-2 rounded-lg transition-colors ${
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-2 text-slate-600 hover:text-slate-900 transition-opacity"
              >
