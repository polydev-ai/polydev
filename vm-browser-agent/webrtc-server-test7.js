...
`;
  ...
  /**
   * GSTREAMER TEST: Test 7: vp8enc with target-bitrate only
   * Pipeline: ximagesrc ! video/x-raw,framerate=30/1 ! vp8enc target-bitrate=2000000 ! fakesink
   * Tests if target-bitrate property is valid
   */
    ...
    const gstPipeline = [
      'gst-launch-1.0',
      '-v',
      `ximagesrc ! video/x-raw,framerate=30/1 ! vp8enc target-bitrate=2000000 ! fakesink`
    ];
