...
router.post('/start', async (req, res) => {
  try {
    const { userId, provider, webrtcOffer } = req.body;

    // DEBUG: Log full request body to diagnose offer loss
    logger.info('[DEBUG] POST /start request body keys', {
      keys: Object.keys(req.body),
      bodySize: JSON.stringify(req.body).length,
      hasWebrtcOffer: !!webrtcOffer,
      webrtcOfferKeys: webrtcOffer ? Object.keys(webrtcOffer) : null
    });
    ...
    logger.info('[RACE-FIX] Starting authentication', {
      userId,
      provider,
      hasWebrtcOffer: !!webrtcOffer
    });

    const result = await browserVMAuth.startAuthentication(userId, provider, webrtcOffer);
 ...
 * Update session heartbeat to keep VM alive and refresh Redis TTL
 */
router.post('/session/:sessionId/heartbeat', async (req, res) => {
  try {
    ...
    // Refresh Redis session TTL to prevent expiration during active use
    try {
      await redisClient.refreshSession(sessionId);
      logger.debug('[SESSION-HEARTBEAT] Redis TTL refreshed', { sessionId });
    } catch (redisError) {
      // Log but don't fail - Redis refresh is best-effort
      logger.warn('[SESSION-HEARTBEAT] Failed to refresh Redis TTL', {
        sessionId,
...
router.get('/sessions/:userId', async (req, res) => {
  try {
    ...
    logger.info('[MULTI-VM] Listing active sessions for user', { userId });

    // Get active sessions from database
    const dbSessions = await db.authSessions.listByUserId(userId);

    // Enrich with real-time status from Redis
    const enrichedSessions = await Promise.all(
      dbSessions.map(async (dbSession) => {
        const redisSession = await browserVMAuth.getSessionStatus(dbSession.session_id);

        return {
          sessionId: dbSession.session_id,
          provider: dbSession.provider,
          status: redisSession?.status || dbSession.status,
          browserIP: redisSession?.browserIP || dbSession.vm_ip,
          browserVMId: redisSession?.browserVMId || dbSession.browser_vm_id,
          novncURL: redisSession?.novncURL,
          createdAt: dbSession.created_at,
          isActive: !!redisSession // True if session is still in Redis (VM is alive)
        };
      })
    );

    // Filter to only show truly active sessions (in Redis)
    const activeSessions = enrichedSessions.filter(s => s.isActive);

    logger.info('[MULTI-VM] Active sessions found', {
      userId,
    ...
    logger.error('[MULTI-VM] List sessions failed', {
