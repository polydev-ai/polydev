...
const config = {
  ...
  // Redis (for ephemeral session state with TTL)
  redis: {
    url: process.env.REDIS_URL || 'redis://localhost:6379',
    sessionTTL: parseInt(process.env.REDIS_SESSION_TTL, 10) || 1800, // 30 minutes default (increased from 10 min for OAuth flows)
  ...
  performance: {
    ...
    browserVmHealthTimeoutMs: parseInt(process.env.BROWSER_VM_HEALTH_TIMEOUT_MS, 10) || 180000, // 3 minutes - increased for TigerVNC startup (45s delay + 30s TigerVNC)
    cliOAuthStartTimeoutMs: parseInt(process.env.CLI_OAUTH_START_TIMEOUT_MS, 10) || 300000, // 5 minutes - timeout for starting OAuth flow in VM (user completes OAuth manually)
    oauthFlowTimeoutMs: parseInt(process.env.OAUTH_FLOW_TIMEOUT_MS, 10) || 300000 // 5 minutes - total timeout for OAuth flow completion
  },
  ...
  debug: {
    ...
    // Skip connectivity diagnostics (curl tests) to speed up OAuth flow
    // These diagnostics can take up to 20 seconds, causing timeouts
    skipConnectivityDiagnostics: process.env.SKIP_CONNECTIVITY_DIAGNOSTICS !== 'false' // Default: true (skip them)
  },
