...
const db = {
  ...
  authSessions: {
    ...
    async upsert(sessionId, updates) {
     ...
     * Returns sessions that are not expired, cancelled, or completed
     */
    async listByUserId(userId) {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', userId)
        .in('status', ['started', 'vm_created', 'ready', 'authenticating', 'waiting', 'pending'])
