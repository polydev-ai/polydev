...
class VMManager {
  ...
  /**
   * Remove TAP device
   * FIX: Derive TAP name from vmId instead of relying on in-memory Map
   * This ensures TAPs are cleaned up even after master controller restart
   */
  async removeTAPDevice(vmId) {
    // Derive TAP name from vmId (same logic as createTAPDevice)
    // This fixes the bug where TAPs accumulated because the Map was lost on restart
    const tapName = this.tapDevices.get(vmId) || `fc-${vmId.substring(0, 8)}`;
    ...
    try {
      ...
      // Only warn if it's not a "device not found" error (already cleaned up)
  ...
  async createVMConfig(vmId, vmType, tapDevice, ipAddress, decodoPort = null) {
    ...
    const vmConfig = {
      'boot-source': {
        ...
        boot_args: `console=ttyS0 reboot=k panic=1 root=/dev/vda rw rootfstype=ext4 rootwait ip=${ipAddress}::192.168.100.1:255.255.255.0::eth0:off net.ifnames=0 biosdevname=0 random.trust_cpu=on gso_max_size=0 ipv6.disable=1${decodoPort ? ' decodo_port=' + decodoPort : ''}`
      },
      'drives': [
        {
          drive_id: 'rootfs',
      ...
      'console': {
        ...
        backend: {
          type: 'file',
          path: path.join(vmDir, 'console.log')
        }
      }
    };
  ...
  /**
   * Configure VM network IP address in systemd-networkd
   */
  async configureVMNetworkIP(vmId, rootfsPath, ipAddress) {
    const mountPoint = `/tmp/vm-network-${vmId}`;

    try {
      logger.info('[NETWORK-CONFIG] Configuring IP address in systemd-networkd', { vmId, ipAddress });
      ...
      logger.info('[NETWORK-CONFIG] Rootfs mounted', { vmId, mountPoint });
      ...
      logger.info('[NETWORK-CONFIG] IP address configured', { vmId, ipAddress, configPath: networkConfigPath });

      // CRITICAL FIX: Sync filesystem before unmount to prevent EXT4 corruption
      // Without sync, sed writes may not be flushed to disk before unmount,
      // causing directory block checksum errors when VM boots
      logger.info('[NETWORK-CONFIG] Syncing filesystem before unmount...', { vmId });
      execSync(`sync`, { stdio: 'pipe' });
      logger.info('[NETWORK-CONFIG] Sync completed', { vmId });

      // Unmount
      execSync(`umount "${mountPoint}"`, { stdio: 'pipe' });
      execSync(`rmdir "${mountPoint}"`, { stdio: 'pipe' });
      logger.info('[NETWORK-CONFIG] Network configuration complete', { vmId, ipAddress });

    } catch (error) {
      logger.error('[NETWORK-CONFIG] Failed to configure network IP', { vmId, ipAddress, error: error.message });
      // Try to cleanup mount on error
      try {
        execSync(`umount "${mountPoint}" 2>/dev/null || true`, { stdio: 'pipe' });
        execSync(`rmdir "${mountPoint}" 2>/dev/null || true`, { stdio: 'pipe' });
      } catch (cleanupErr) {
        // Ignore cleanup errors
      }
      throw new Error(`Network IP configuration failed: ${error.message}`);
    }
  }
  ...
  async cloneGoldenSnapshot(vmId, vmType = 'cli', userId = null, sessionId = null, ipAddress = null) {
    ...
    try {
      // FULL copy of rootfs (not CoW) to ensure golden rootfs modifications persist
      // Using dd instead of cp --reflink to get actual file contents, not snapshot reference
      ...
      logger.info('[CLONE-SNAPSHOT] Using dd for full copy (not CoW) to preserve golden rootfs modifications', {
        vmId,
        src: rootfsSrc,
        dst: rootfsDst
      });
      ...
      // CRITICAL: Run e2fsck BEFORE any modifications to ensure clean filesystem
      logger.info('[CLONE-SNAPSHOT] Running pre-modification filesystem check...', { vmId, rootfsDst });
      try {
        execSync(`e2fsck -y -f "${rootfsDst}"`, { stdio: 'pipe' });
        logger.info('[CLONE-SNAPSHOT] Pre-modification filesystem check completed', { vmId });
      } catch (error) {
        // e2fsck returns non-zero exit code even when it successfully repairs
        logger.info('[CLONE-SNAPSHOT] e2fsck completed (non-zero exit is normal for repairs)', { vmId });
      }

      // Configure network IP address in systemd-networkd
      if (ipAddress) {
      ...
      // CRITICAL FIX: Run e2fsck AGAIN after modifications to ensure consistency
      // The "No space for directory leaf checksum" error occurs because multiple mount/unmount operations
      // can damage the ext4 extent tree even with sync. e2fsck -y -f repairs these structural issues.
      logger.info('[CLONE-SNAPSHOT] Running filesystem check to repair any corruption...', { vmId, rootfsDst });
      try {
        execSync(`e2fsck -y -f "${rootfsDst}"`, { stdio: 'pipe' });
        logger.info('[CLONE-SNAPSHOT] Filesystem check completed successfully', { vmId });
      } catch (error) {
        // e2fsck returns non-zero exit code even when it successfully repairs the filesystem
        // Exit codes 1-2 mean filesystem was modified/repaired (success)
        // Exit codes 4+ indicate actual failures
        if (error.status >= 4) {
          logger.error('[CLONE-SNAPSHOT] Filesystem check failed', { vmId, exitCode: error.status, error: error.message });
          throw new Error(`Filesystem check failed: ${error.message}`);
        } else {
          logger.info('[CLONE-SNAPSHOT] Filesystem repaired successfully', { vmId, exitCode: error.status });
        }
      }
  ...
  async injectOAuthAgent(vmId, rootfsPath, userId = null, sessionId = null) {
    ...
    try {
      ...
      // The proxy works for statsig.anthropic.com but returns ERR_INVALID_IP_ADDRESS for api.anthropic.com
      // This appears to be intentional blocking of AI API endpoints by the residential proxy service
      // VMs will use direct NAT connection instead (all VMs share host's external IP)
      let proxyEnvVars = '';
      logger.info('[INJECT-AGENT] Using direct NAT connection (Decodo proxy disabled - blocks api.anthropic.com)', { vmId });
...
${proxyEnvVars}`;
      ...
      logger.info('[INJECT-AGENT] Environment injected', {
        vmId,
        userId,
        sessionId,
        envPath,
        hasProxy: !!proxyEnvVars
      });
      ...
      // Copy gstreamer-webrtc-controller.js (required by webrtc-server.js)
      try {
        logger.info('[INJECT-AGENT] Copying gstreamer-webrtc-controller.js...');
        execSync(`cp "${path.join(srcAgentDir, 'gstreamer-webrtc-controller.js')}" "${agentDir}/"`, { stdio: 'inherit' });
        logger.info('[INJECT-AGENT] gstreamer-webrtc-controller.js copied successfully');
      } catch (err) {
        logger.error('[INJECT-AGENT] Failed to copy gstreamer-webrtc-controller.js', { error: err.message });
        throw err;
      }

      // Copy gstreamer-webrtc-helper.py (required by gstreamer-webrtc-controller.js)
      try {
        logger.info('[INJECT-AGENT] Copying gstreamer-webrtc-helper.py...');
        execSync(`cp "${path.join(srcAgentDir, 'gstreamer-webrtc-helper.py')}" "${agentDir}/"`, { stdio: 'inherit' });
        logger.info('[INJECT-AGENT] gstreamer-webrtc-helper.py copied successfully');
      } catch (err) {
        logger.error('[INJECT-AGENT] Failed to copy gstreamer-webrtc-helper.py', { error: err.message });
        throw err;
      }

      // Make gstreamer-webrtc-helper.py executable
      try {
        logger.info('[INJECT-AGENT] Making gstreamer-webrtc-helper.py executable...');
        execSync(`chmod +x "${agentDir}/gstreamer-webrtc-helper.py"`, { stdio: 'inherit' });
        logger.info('[INJECT-AGENT] gstreamer-webrtc-helper.py made executable');
      } catch (err) {
        logger.error('[INJECT-AGENT] Failed to chmod gstreamer-webrtc-helper.py', { error: err.message });
        throw err;
      }

      logger.info('[INJECT-AGENT] Agent files copied (OAuth agent + WebRTC server + webrtcbin dependencies + bundled Node.js)', { vmId });

      // Create supervisor script for production golden rootfs
      // Golden rootfs has systemd services for: xvfb, xfce-session
      // Supervisor manages: x11vnc (manual start), OAuth agent, WebRTC server
      // NOTE: x11vnc in systemd wasn't working, so we start it manually after XFCE is ready
...
echo "[SUPERVISOR] \$(date -Is) Production supervisor starting..." | tee -a "$LOG_DIR/supervisor.log"
echo "[SUPERVISOR] SESSION_ID=\${SESSION_ID:-<unset>} DISPLAY=\${DISPLAY:-:1}" | tee -a "$LOG_DIR/supervisor.log"
...
# Wait for systemd services (TigerVNC will start automatically)
echo "[SUPERVISOR] Waiting for systemd services to start..." | tee -a "$LOG_DIR/supervisor.log"
sleep 5

# Verify TigerVNC systemd service is running
for service in tigervnc; do
  if systemctl is-active --quiet \$service.service 2>/dev/null; then
    echo "[SUPERVISOR] ✓ \$service.service is running (TigerVNC)" | tee -a "$LOG_DIR/supervisor.log"
  else
    echo "[SUPERVISOR] ⚠ \$service.service is NOT running - attempting to start..." | tee -a "$LOG_DIR/supervisor.log"
    systemctl start \$service.service 2>&1 | tee -a "$LOG_DIR/supervisor.log" || true
    sleep 5
  fi
done

# TigerVNC handles its own X server (Xvnc) - no need for Xvfb or manual VNC start
echo "[SUPERVISOR] TigerVNC systemd service handles VNC server - no manual start needed" | tee -a "$LOG_DIR/supervisor.log"
sleep 3

echo "[SUPERVISOR] Starting OAuth agent and WebRTC server..." | tee -a "$LOG_DIR/supervisor.log"

# Start OAuth agent with logging (console + file via tee)
...
# Start WebRTC server with logging (console + file via tee)
...
cleanup() {
  ...
  kill "$VNC_PID" "$OAUTH_PID" "$WEBRTC_PID" 2>/dev/null || true
  wait "$VNC_PID" "$OAUTH_PID" "$WEBRTC_PID" 2>/dev/null || true
...
# Monitor OAuth and WebRTC services (TigerVNC systemd handles VNC auto-restart)
echo "[SUPERVISOR] All services started successfully" | tee -a "$LOG_DIR/supervisor.log"
while [ "$SHUTTING_DOWN" -eq 0 ]; do
  # TigerVNC systemd service handles VNC - no manual monitoring needed
...
`;
      ...
      // Create systemd service file (runs supervisor that manages OAuth + WebRTC)
      // AI-validated: TigerVNC systemd service handles X server + VNC + XFCE (via xstartup)
...
`;
      ...
      // Terminal autostart is already configured in golden rootfs at:
      // /etc/xdg/autostart/terminal.desktop
      // No need to create it at runtime
      logger.info('[INJECT-AGENT] Terminal autostart already in golden rootfs', { vmId });

      // CRITICAL: Force filesystem flush before unmount to ensure persistence
      logger.info('[INJECT-AGENT] Flushing filesystem writes...', { mountPoint });
      execSync(`sync`, { stdio: 'inherit' });
      execSync(`sync`, { stdio: 'inherit' }); // Double sync for safety
      logger.info('[INJECT-AGENT] Filesystem synced');
      ...
      // Always unmount
      try {
        logger.info('[INJECT-AGENT] Final sync before unmount...', { mountPoint });
        execSync(`sync`, { stdio: 'inherit' });

        logger.info('[INJECT-AGENT] Unmounting rootfs with sync option...', { mountPoint });
  ...
  async createVM(userId, vmType, decodoPort = null, decodoIP = null, sessionId = null) {
    ...
    try {
      ...
      await this.withTimeout(
        this.cloneGoldenSnapshot(vmId, vmType, userId, sessionId, ipAddress),
        90000, // 90 seconds - production rootfs is 6.8GB (increased from 15s)
  ...
  async processCleanupTasks() {
    ...
    for (const task of tasks) {
      try {
        ...
        // Destroy the VM (always preserve logs for debugging failed VMs)
        await this.destroyVM(task.vm_id, true, true);
