...
function launchBrowserForSession(sessionId, oauthUrl) {
  ...
  try {
    ...
    const proxyConfig = (session && session.proxyConfig) ? session.proxyConfig : {};
    ...
    if (proxyConfig && proxyConfig.httpProxy && !isFirefox) {
      env.HTTP_PROXY = proxyConfig.httpProxy;
      env.ALL_PROXY = proxyConfig.httpProxy;
    }
    if (!isFirefox) {
      if (proxyConfig && proxyConfig.httpsProxy) {
        env.HTTPS_PROXY = proxyConfig.httpsProxy;
      } else if (proxyConfig && proxyConfig.httpProxy) {
        env.HTTPS_PROXY = proxyConfig.httpProxy;
      }
      if (proxyConfig && proxyConfig.noProxy) {
...
process.on('uncaughtException', (error) => {
  logger.error('Uncaught exception', { error: error && error.message, stack: error && error.stack });
});
...
async function handleStartCLIAuth(req, res, provider) {
  ...
  try {
    ...
    logger.error('Failed to parse auth payload', { provider, error: error.message, rawBody: body && body.slice ? body.slice(0, 500) : body });
  ...
  const debugOptions = (payload && payload.debug) || (payload && payload.debugOptions) || {};
  const runStrace = (debugOptions && debugOptions.strace === true) || (debugOptions && debugOptions.runStrace === true);
  const runExtraDiagnostics = !(debugOptions && debugOptions.skipConnectivityChecks === true);
...
`;
  ...
  if (provider === 'claude_code') {
    try {
      ...
      // Use local dnsmasq (127.0.0.1) which filters AAAA records to force IPv4
      // This fixes ERR_INVALID_IP_ADDRESS when Node.js tries to connect to IPv6 addresses
      // that aren't routable via the Firecracker VM NAT (which only supports IPv4)
      const resolvContent = 'nameserver 127.0.0.1\n';
      await fs.writeFile(resolvPath, resolvContent, 'utf-8');
      logger.info('Overrode /etc/resolv.conf to use local dnsmasq (filters AAAA for IPv4-only)', { provider, sessionId, resolvContent });

      // Block ALL IPv6 traffic at the kernel level using ip6tables
      // This is a more aggressive fix that ensures Node.js c-ares cannot use IPv6 addresses
      // even if DNS returns them (dnsmasq filter-AAAA may not be working)
      try {
        execSync('ip6tables -P OUTPUT DROP 2>/dev/null || true', { stdio: 'ignore' });
        execSync('ip6tables -P INPUT DROP 2>/dev/null || true', { stdio: 'ignore' });
        execSync('ip6tables -P FORWARD DROP 2>/dev/null || true', { stdio: 'ignore' });
        logger.info('Blocked all IPv6 traffic via ip6tables', { provider, sessionId });
      } catch (ip6Error) {
        logger.warn('Failed to block IPv6 via ip6tables (may not be fatal)', { provider, sessionId, error: ip6Error.message });
      }
    } catch (error) {
    ...
    // DNS diagnostics ALWAYS run - critical for debugging ERR_INVALID_IP_ADDRESS
    runDiagnostic('dnsmasq listening on port 53', 'ss -lntu | grep :53 || netstat -ln | grep :53');
    runDiagnostic('dnsmasq query api.anthropic.com', 'dig @127.0.0.1 api.anthropic.com A +short +time=5 2>&1 || nslookup api.anthropic.com 127.0.0.1 2>&1 || host api.anthropic.com 127.0.0.1 2>&1');
    runDiagnostic('Node.js dns.lookup test', 'node -e "require(\'dns\').lookup(\'api.anthropic.com\', {family:4}, (e,a)=>console.log(e||a))"');
    runDiagnostic('Node.js dns.resolve4 test', 'node -e "require(\'dns\').resolve4(\'api.anthropic.com\', (e,a)=>console.log(e||a))"');
    runDiagnostic('Node.js dns.resolveAny test', 'node -e "require(\'dns\').resolveAny(\'api.anthropic.com\', (e,a)=>console.log(JSON.stringify(e||a)))"');
    runDiagnostic('getent hosts check', 'getent ahosts api.anthropic.com 2>&1 | head -5');
    runDiagnostic('resolv.conf content', 'cat /etc/resolv.conf');
    // Test fetch/undici (Node.js v20 built-in)
    runDiagnostic('Node.js fetch test', 'node -e "fetch(\'https://api.anthropic.com\').then(r=>console.log(\'OK:\',r.status)).catch(e=>console.log(\'ERR:\',e.code,e.message))"');
    // Test direct IP connection
    runDiagnostic('Direct TCP to 160.79.104.10:443', 'nc -zvw5 160.79.104.10 443 2>&1 || echo "Failed"');
    // Check network interfaces and routes
    runDiagnostic('Network interfaces', 'ip addr show eth0 2>&1 | head -10');
    runDiagnostic('Default route', 'ip route show default');
    // Check IPv6 status
    runDiagnostic('IPv6 status', 'cat /proc/sys/net/ipv6/conf/all/disable_ipv6 2>/dev/null && echo "IPv6 disabled" || echo "IPv6 enabled"');

    if (debugOptions && debugOptions.skipConnectivityChecks === true) {
      logger.info('Skipping connectivity diagnostics (but DNS tests ran)', { provider, sessionId });
    } else {
...
`;
  ...
  // Use a pseudo-terminal for Claude CLI so it behaves like an interactive session
    ...
    scriptLogPath = `/tmp/${provider}-login-${sessionId}.log`;
    const joinedArgs = [cliCommand, ...cliArgs].join(' ');
    // Disable IPv6 at runtime via sysctl (VM NAT only supports IPv4)
    // This fixes "ERR_INVALID_IP_ADDRESS" when Node.js tries to use IPv6 addresses from DNS
    const disableIpv6 = "sysctl -w net.ipv6.conf.all.disable_ipv6=1 net.ipv6.conf.default.disable_ipv6=1 >/dev/null 2>&1";
    // Force IPv4-only DNS resolution - NO proxy or preload needed
    // dnsmasq + /etc/hosts (without IPv6 entries) provide IPv4-only resolution at system level
    // Add NODE_DEBUG=net,undici to capture socket connection details for debugging
    const cmdWithNodeOptions = `${disableIpv6}; export NODE_DEBUG='net,undici' NODE_OPTIONS='--dns-result-order=ipv4first' && ${joinedArgs}`;
    commandParts = ['script', '-q', '--return', '-c', cmdWithNodeOptions, scriptLogPath];
  }
  ...
  const cliProcess = spawn(command, args, {
    env: {
      ...
      NODE_OPTIONS: '--dns-result-order=ipv4first',  // Force IPv4 DNS resolution (NO proxy needed)
      NODE_DEBUG: 'net,undici'  // Debug socket connections to find ERR_INVALID_IP_ADDRESS source
    },
    stdio: ['pipe', 'pipe', 'pipe']
  });

  cliProcess.on('error', (error) => {
    logger.error('CLI process failed to spawn', { provider, sessionId, error: error && error.message, stack: error && error.stack });
  });
  ...
  const logOutput = (data) => {
    ...
    if (session && session.automation) {
      ...
      if (!session.automation.themeAccepted && text.includes('Choose the text style')) {
        ...
        setTimeout(() => {
          try {
            if (cliProcess.stdin && cliProcess.stdin.write) {
...
async function handleOAuthCallback(req, res) {
  ...
  try {
    ...
    // Use native http.get() instead of fetch() for Node.js v12 compatibility
    const proxyRes = await new Promise((resolve, reject) => {
      const requestTimeout = setTimeout(() => {
        reject(new Error('Proxy request timeout after 10 seconds'));
      }, 10000);

      const proxyReq = http.get(proxyUrl, {
        headers: {
        ...
        proxyResponse.on('end', () => {
          resolve({
            ...
            contentType: proxyResponse.headers['content-type'] || 'text/html',
            text: responseData
          });
        });
      });
...
server.on('error', (error) => {
  logger.error('HTTP server error', { error: error && error.message, stack: error && error.stack });
});

server.on('clientError', (error, socket) => {
  logger.warn('HTTP client error', { error: error && error.message });
  if (socket && socket.end) {
    try {
      socket.end('HTTP/1.1 400 Bad Request\r\n\r\n');
    } catch (err) {
      logger.warn('Failed to close socket after client error', { error: err && err.message });
    }
  }
});
